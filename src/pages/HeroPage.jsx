...
const HeroPage = () => {
  ...
  // ✅ Navigate to register-team and send tournament data
    ...
    <>
      ...
      <div className="relative z-10 text-white font-body">
        ...
        <section className="w-full h-screen flex flex-col justify-center items-center text-center px-4">
          <h1 className="text-6xl md:text-8xl font-extrabold text-yellow-400 mb-4 font-heading drop-shadow-lg animate-pulse">
            WarSky ESports
          </h1>
          <p className="text-2xl md:text-3xl text-white/90 mb-6 font-body">
        ...
        >
          <h2 className="text-5xl font-bold text-yellow-400 mb-12 font-heading">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl w-full">
            {tournaments.length === 0 ? (
              ...
              tournaments.map((t) => (
                ...
                >
                  <h3 className="text-2xl font-bold text-yellow-400 mb-2">{t.name}</h3>
                  <p className="text-white mb-1 font-body">Entry Fee: {t.entryFee}</p>
                  <p className="text-white mb-1 font-body">
                    Start: {new Date(t.startDate).toLocaleDateString()} | End:{" "}
                    {new Date(t.endDate).toLocaleDateString()}
                  </p>
                  <p className="text-white mb-4 font-body">Prize: {t.prizePool}</p>
                  {t.rules.length > 0 && (
                  ...
                  <p className="text-white mb-4 font-body">Status: {t.status}</p>
                  <button
        ...
        <div className="fixed right-0 top-1/3 flex flex-col gap-4 z-50">
          <a
            ...
            className="bg-pink-500 px-4 py-3 rounded-l-full shadow-lg hover:bg-pink-600 transition"
          >
            Instagram
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-red-600 px-4 py-3 rounded-l-full shadow-lg hover:bg-red-700 transition"
          >
            YouTube
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-green-500 px-4 py-3 rounded-l-full shadow-lg hover:bg-green-600 transition"
          >
            WhatsApp
          </a>
        </div>
